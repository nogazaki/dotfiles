...
local assets_path = capi.awesome.conffile:match '^(.-)rc%.lua$'
   .. 'pretty/assets/'
...
network_service:connect_signal('network::type', function(_, connection_type)
   if connection_type:match 'wireless' then
      ...
      icon.stylesheet =
         string.format('svg { fill:%s; }', beautiful.xcolor3 .. '44')
   else
...
network_service:connect_signal('network::state', function(_, state)
   ...
   icon.visible = indicator
end)
network_service:connect_signal(
   'wireless::active_access_point',
   function(_, _, strength)
