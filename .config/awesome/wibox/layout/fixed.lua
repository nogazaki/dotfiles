...
function fixed:fit(context, orig_width, orig_height)
    if #self._private.widgets == 0 then return 0, 0 end
    ...
    for _, widget in ipairs(self._private.widgets) do
        ...
        if is_y then
            used_main = used_main + h
            used_second = math.max(used_second, w)

            height_left = orig_height - used_main
        else
            used_main = used_main + w
            used_second = math.max(used_second, h)

            width_left = orig_width - used_main
        end

        if used_main >= (is_y and orig_height or orig_width) then
            -- Out of places
            break
        end
    end
    ...
    if is_y then
        return used_second, math.min(used_main, orig_height)
    else
        return math.min(used_main, orig_width), used_second
    end
